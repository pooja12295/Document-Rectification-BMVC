# python3 inference.py --seg_model_path ./model_pretrained/seg.pth --rec_model_path ./model_pretrained/DocAttentionRect.pth --distorrted_path ./distorted/ --rectified_path ./rectified/
from model import DocAttentionRect
...
def reload_seg_model(model, path="", device="cpu"):
    if not bool(path):
        return model
    else:
...
def reload_rec_model(model, path="", device="cpu"):
    if not bool(path):
        return model
    else:
        model_dict = model.state_dict()
...
def rec(seg_model_path, rec_model_path, distorrted_path, save_path, device="cpu"):
    ...
    reload_seg_model(net.msk, seg_model_path, device=device)
   
    reload_rec_model(net.bm, rec_model_path, device=device)
...
def main():
    ...
    parser.add_argument('--rec_model_path', default='./model_pretrained/DocAttentionRect.pth')
